# FILE: llm_dynamic_game.py (Corrected for Case-Insensitive Comparison)
...
def run_challenge(state, challenge):
    ...
    if challenge_type == 'QTE':
        ...
        # --- THIS IS THE BUG FIX ---
        # 1. We take the user's input and convert it to uppercase.
        user_input_upper = input("> ").strip().upper().split()
        
        # 2. We create a new list from the correct answer, also in uppercase.
        correct_sequence_upper = [item.upper() for item in sequence]
        
        # 3. We compare the two uppercase lists. This is a fair comparison.
        if user_input_upper == correct_sequence_upper:
            ...
            # This 'else' block will now only be triggered if the words are actually wrong.
